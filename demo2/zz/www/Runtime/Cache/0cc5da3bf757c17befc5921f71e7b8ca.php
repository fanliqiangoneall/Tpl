...
<script>
    ...
    document.getEmementById('gx1').innerHTML = '<div id="divA"><table class="s"><tr><td></td></tr></table></div>';
...
<div class="prolist_in">
    <?php if(is_array($allclass)): $i = 0; $__LIST__ = array_slice($allclass,0,21,true);if( count($__LIST__)==0 ) : echo "no" ;else: foreach($__LIST__ as $key=>$allclass_left2): $mod = ($i % 2 );++$i;?><li><a style="cursor:pointer;" href="<?php echo U('/product_category','','');?>/<?php echo getprocatename($allclass_left2['pcid']);?>"><?php echo ($allclass_left2['class_name']); ?></a></li><?php endforeach; endif; else: echo "no" ;endif; ?>
</div>

<div class="r_mbx"><img src="__TMPL__demo2/img/in_home.jpg" border="0" style="vertical-align:middle;" />&nbsp;<a href="<?php echo U('/');?>">Home</a>&nbsp;>&nbsp;<a href="<?php echo U('/product');?>">Products</a><?php if(is_array($path)): $i = 0; $__LIST__ = $path;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$path): $mod = ($i % 2 );++$i;?>&nbsp;>&nbsp;<a href="<?php echo U('/product_category','','');?>/<?php echo getprocatename($path['pcid']);?>"><?php echo ($path['class_name']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>&nbsp;>&nbsp;<a href="<?php echo U('/product_category','','');?>/<?php echo getprocatename($procate['pcid']);?>"><?php echo ($procate['class_name']); ?></a></div>
  ...
  <div class="subNavBox" style="width: 263px;">
    <div style="width: 220px;margin: auto;">
     ...
     <li style="font-size: 16px;line-height: 26px;margin-bottom: 5px">
       <span style="font-weight: bold;font-size: 18px">Tel: </span><br /><?php echo chaifen($about['mobile'],0);?>
     </li>
...
<div class="page_Main">
  <div class="page_RightTitle"><span><?php echo ($product['pro_name']); ?></span></div>
    <div id="product_detail">
          <div class="left">
            <div class="big_pic" id="gallery"><a href="<?php echo getmainimgurl($product['main_img']);?>" title="<?php echo ($product['pro_name']); ?>"><img src="<?php echo getmainimgurl($product['main_img']);?>" alt="<?php echo getmainimgurl($product['main_img']);?>"  <?php if($product['proalt'] != ''): ?>title="<?php echo ($product['proalt']); ?>"<?php else: ?>title="<?php echo ($product['pro_name']); ?>"<?php endif; ?> class="jqzoom"></a> <a href="#" title="<?php echo ($product['pro_name']); ?>"></a></div>
          </div>
          <div class="left product_info">
            <div class="pro_info_box">
              <ul>
                <li style="background:url(__TMPL__demo2/img/in_mail.jpg) left no-repeat; padding-left:44px; line-height:34px;font-size: 18px;color: #4c4c4c">E-mail: <a href="mailto:<?php echo chaifen($about['email'],0);?>" onFocus="blur()" onClick="turn_sta(3,5);gtag_report_conversion_email();" style="font-size:18px; color:#333333;"><?php echo chaifen($about['email'],0);?></a></li>
                <li style="background:url(__TMPL__demo2/img/in_share.jpg) left no-repeat; padding-left:44px; line-height:41px;"><span style="float:left;font-size: 18px;color: #4c4c4c">Follow us:</span><div style="float:left; margin-left:5px; margin-top:12px;"><!--[if gt IE 8]>
      <!-- Go to www.addthis.com/dashboard to customize your tools -->
  <div class="addthis_sharing_toolbox"></div>
  <script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-573a7b508824f77c"></script>
  <![endif]--></div><div class="clear"></div></li>
                <li style="padding-top:15px;"><a onClick="javascript:jumpto();" style="width:120px; display:block; font-size:18px; text-align:center; line-height:40px; cursor:pointer; height:40px; background:#6ac2b3; color:#FFFFFF;">Inquiry Now</a></li>
              </ul>
            </div>
            <ul class="pic_list smallImgList">
                <div class="mod18"><span id="prev" class="btn prev"></span> <span id="next" class="btn next"></span>
                  <div id="listBox" class="listBox">
                    <ul class="cf">
                       <li class="hover"><a href="<?php echo getmainimgurl($product['main_img']);?>" title="<?php echo ($product['pro_name']); ?>"><img src="<?php echo getmainimgurl($product['main_img']);?>"  <?php if($pro['proalt'] != ''): ?>alt="<?php echo ($product['proalt']); ?>"<?php else: ?>alt="<?php echo ($product['pro_name']); ?>"<?php endif; ?> class="jqzoom"></a></li>
                      <?php if(is_array($proimg)): $i = 0; $__LIST__ = $proimg;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$proimg): $mod = ($i % 2 );++$i; if(getfimgurl($proimg['field_auxiliary_fid']) != '/Public/img/nopic.gif'): ?><li><a href="<?php echo getfimgurl($proimg['field_auxiliary_fid']);?>" ><img src="<?php echo getfimgurl($proimg['field_auxiliary_fid']);?>" class="jqzoom" <?php if($pro['proalt'] != ''): ?>alt="<?php echo ($product['proalt']); ?>"<?php else: ?>alt="<?php echo ($product['pro_name']); ?>"<?php endif; ?>/></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                  </div>
                </div>
                <script type="text/javascript">
    ...
    { 
      ...
      break;
        ...
        <div class="pro_detail" id="gx1" style="display:block;"><?php echo str_replace('&nbsp;',' ',htmlspecialchars_decode($product['pro_profile']));?></div>
...
<div class="clear"></div>
    ...
    <?php if($otherpro != ''): ?><div class="page_RightTitle">Other Products</div>
    <div class="page_RightMain" style="padding-bottom:0;">
       <?php if(is_array($otherpro)): $i = 0; $__LIST__ = array_slice($otherpro,0,3,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$otherpro): $mod = ($i % 2 );++$i;?><div class="asd2">
                     <div class="img2" ><a href="<?php echo U('/Product','','');?>/<?php echo getproname($otherpro['pid']);?>"><img src="<?php echo getpcimgurl($otherpro['main_img']);?>" <?php if($otherpro['proalt'] != ''): ?>alt="<?php echo ($otherpro['proalt']); ?>"<?php else: ?>alt="<?php echo ($otherpro['pro_name']); ?>"<?php endif; ?> /></a>
                  </div>
 
                <div class="in_proname2">
                   <a href="<?php echo U('/Product','','');?>/<?php echo getproname($otherpro['pid']);?>"><?php echo ($otherpro['pro_name']); ?></a>
                </div>
               </div>
    ...
    <?php if($newpro != ''): ?><div class="page_RightTitle">New Products</div>
    <div class="page_RightMain" style="padding-bottom:0;">
     <?php if(is_array($newpro)): $i = 0; $__LIST__ = array_slice($newpro,0,3,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newpro): $mod = ($i % 2 );++$i;?><div class="asd2">
                     <div class="img2" ><a href="<?php echo U('/Product','','');?>/<?php echo getproname($newpro['pid']);?>"><img src="<?php echo getpcimgurl($newpro['main_img']);?>" <?php if($newpro['proalt'] != ''): ?>alt="<?php echo ($newpro['proalt']); ?>"<?php else: ?>alt="<?php echo ($newpro['pro_name']); ?>"<?php endif; ?> /></a>
                  </div>
 
                <div class="in_proname2">
                   <a href="<?php echo U('/Product','','');?>/<?php echo getproname($newpro['pid']);?>"><?php echo ($newpro['pro_name']); ?></a>
                </div>
               </div>
               <?php if(($key+1)%3 == 0): ?><div style="clear:both; width: 100%; height:26px;"></div>
               <?php else: ?>
