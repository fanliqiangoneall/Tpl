...
<!--logo代码块开始-->
	<div class="logo">
    	<div class="logok">
        	<a href="<?php echo U('/');?>"><img src="<?php echo ($logo); ?>" alt="<?php echo ($company_info['logo_title']); ?>" title="<?php echo ($company_info['logo_title']); ?>" border="0"/></a>
            <div class="logor">
            	<ul class="logor-1">
                	<li><img src="__TMPL__demo2/img/down.png"  style="padding-left:10px;" /><a href="<?php echo U('/download');?>">资料下载&nbsp;&nbsp;&nbsp;&nbsp;</a><div class="clear"></div></li>
                    <li><img src="__TMPL__demo2/img/logoxian.jpg" />&nbsp;&nbsp;</li>
                    <li style="width:230px;"><img src="__TMPL__demo2/img/email.png" /><a href="mailto:<?php echo ($about['email']); ?>">邮件：<?php echo ($about['email']); ?>&nbsp;&nbsp;&nbsp;&nbsp;</a><div class="clear"></div></li>
                    <li><img src="__TMPL__demo2/img/logoxian.jpg" />&nbsp;&nbsp;</li>
                    <li><img src="__TMPL__demo2/img/tel.png" /><a>联系电话 ：<?php echo ($about['mobile']); ?></a><div class="clear"></div></li>
                    <div class="clear"></div>
                </ul>
<script>
function checksub(){
	if($('#searchin').val()=='输入你想查找的产品关键词'){
		...
		return false;	
                ...
                <div class="search">
                	<form action="<?php echo U('/search');?>" method="post" onsubmit=" return checksub()">
                    	<input id="searchin" type="text" value="输入你想查找的产品关键词" onfocus="if (value=='输入你想查找的产品关键词'){value='';$(this).css('color','#000')}" onblur="if (value==''){value='输入你想查找的产品关键词';$(this).css('color','#bdbdbd');}" name="pro_name" />
                        <input id="searchsubmit" type="submit" value="产品搜索" />
                        <div class="clear"></div>
                    </form>
               </div>
...
<!--banner代码块开始-->
	<div class="banner">
    <link rel="stylesheet" type="text/css" href="__TMPL__demo2/css/flexslider.css?<?php echo time();?>" />
    <script src="__TMPL__demo2/js/jquery.flexslider-min.js"></script>
	...
	$(function() {
        ...
        <div class="flexslider">
            <ul class="slides">
            	<?php if(is_array($banner)): $i = 0; $__LIST__ = $banner;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$banner): $mod = ($i % 2 );++$i;?><li><a href="<?php echo ($banner['ad_link']); ?>"  <?php if($banner['ad_open_type'] != ''): ?>target="_new"<?php endif; ?>><img src="<?php echo getimgurl($banner['ad_pic']);?>" alt="" width="100%" /></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
        </div>
		<div class="clear"></div>
    	<!--<img src="__TMPL__demo2/img/banner.jpg" width="100%" />-->
    </div>
<!--banner代码块结束-->
...
<!--面包屑代码块开始-->
<div class="crumbs"><img src="__TMPL__demo2/img/home.png" />&nbsp;&nbsp;<a href="<?php echo U('/');?>">首页</a> > <a href="<?php echo U('/product');?>">产品中心</a><?php if(is_array($path)): $i = 0; $__LIST__ = $path;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$path): $mod = ($i % 2 );++$i;?>><a href="<?php echo U('/Product_category',getprocatename($path['pcid']));?>"><?php echo ($path['class_name']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
><?php echo ($prolist['class_name']); ?>
</div>
<div class="clear"></div>
<!--代码块结束-->
...
<div class="inproductlist">
	<div class="inprol">
    	<div class="inprolsit">
        	<div class="inprolist-1"><img src="__TMPL__demo2/img/inprodian.jpg" />&nbsp;&nbsp;产品分类<div class="clear"></div></div>
            <div class="inprolist-2">
                <ul class="inprolist-2-1">
                    <?php if(is_array($allclass)): $i = 0; $__LIST__ = $allclass;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$allclass): $mod = ($i % 2 );++$i;?><li class="inprolist-2-parenter"><a href="<?php echo U('/Product_category',getprocatename($allclass['pcid']));?>"><?php echo ($allclass['class_name']); ?></a></li>
                        <?php if($allclass["childs"] != ""): if(is_array($allclass['childs'])): $i = 0; $__LIST__ = $allclass['childs'];if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$childs): $mod = ($i % 2 );++$i;?><li class="inprolist-2-child"><a href="<?php echo U('/Product_category',getprocatename($childs['pcid']));?>"><?php echo ($childs['class_name']); ?></a></li><?php endforeach; endif; else: echo "暂无数据" ;endif; endif; endforeach; endif; else: echo "暂无数据" ;endif; ?>                  
                </ul>
            </div>
        </div>
        <div class="clear"></div>
    ...
    <div class="inpror">
    	<div class="inprocate"><?php echo ($prolist['class_name']); ?><br><?php if($prolist['readme'] != ''): ?><span style="font-size:12px; color:#666;">分类描述:<?php echo ($prolist['readme']); ?></span><?php endif; ?></div>
        ...
        <div class="clear">
        <?php if($system_set['showtype'] == 1 ): ?><div class="pro">
                <div class="prok" style="width:725px !important;">
                    <?php if(is_array($pro)): $i = 0; $__LIST__ = $pro;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$pro): $mod = ($i % 2 );++$i;?><div class="proimg">
                        <a href="<?php echo U('/Product_detail',getproname($pro['pid']));?>"><img src="<?php echo getpcimgurl($pro['main_img']);?>" />
                        <div class="proname"><?php echo ($pro['pro_name']); ?></div></a>
                    </div><?php endforeach; endif; else: echo "暂无数据" ;endif; ?>           
                </div>
            </div>
        <?php else: ?>
            <div class="pro" style="height:auto !important;">
                <div class="prok" style="width:725px !important; height:auto !important">
                    <?php if(is_array($pro)): $i = 0; $__LIST__ = $pro;if( count($__LIST__)==0 ) : echo "暂无数据" ;else: foreach($__LIST__ as $key=>$pro): $mod = ($i % 2 );++$i;?><div style=" border-bottom:1px solid #CCC; margin-bottom:20px;">
                       <a href="<?php echo U('/Product_detail',getproname($pro['pid']));?>"> <img src="<?php echo getpcimgurl($pro['main_img']);?>" style="float:left; width:150px; height:150px;" /></a>
                        <div class="proname" style="float:left; width:70%; text-align:left;padding-top:20px;">产品名称：<?php echo ($pro['pro_name']); ?></div>
                        <div class="proname" style="float:left; width:70%; text-align:left; padding-top:30px;">产品型号：<?php echo ($pro['pro_model']); ?></div>
                        <div class="proname" style="float:left; padding-top:30px;">产品特点简介：<?php echo ($pro['pro_description']); ?></div>
                        <div class="clear" style="width:100%; padding-top:20px;"></div>
                    </div><?php endforeach; endif; else: echo "暂无数据" ;endif; ?>
                    <div class="clear"></div>
                </div>
            </div>
