...
<script>
$(document).ready(function () {
	$('#menu2').removeClass().addClass("menuon1");
});
</script>
<style>
.class_re img{ max-width:100%;}
</style>
<div><img src="__TMPL__WAP/img/banner1.jpg" border="0" width="100%" /></div>
<div data-role="content" style="padding-top:15px;">
    <div class="main_top"><a href="<?php echo U('/');?>">Home</a>&nbsp;>&nbsp;<a href="<?php echo U('/product');?>">Products</a>&nbsp;>&nbsp;<?php echo ($prolist['class_name']); ?></div>
  <div class="lmtitle2"><?php echo ($prolist['class_name']); ?></span></div>
  <?php if($prolist['cate_profile'] != ''): ?><div class="class_re" style="width:94%; margin:auto; margin-top:25px; line-height:25px; font-size:14px;"><?php echo htmlspecialchars_decode($prolist['cate_profile']);?></div><?php endif; ?>
    <div style="width:96%; padding:3% 2%; height:auto;margin-top: 2%;">
    <?php if(is_array($pro)): $k = 0; $__LIST__ = $pro;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pro): $mod = ($k % 2 );++$k;?><div class="page_pro"><a data-ajax="false" href="<?php echo U('/product','','');?>/<?php echo getproname($pro['pid']);?>">
       <div class="page_proimg"><img src="<?php echo getpcimgurl($pro['main_img']);?>" width='100%' height='100%'></div>
       <div class="page_proname"><?php echo ($pro['pro_name']); ?></div></a></div>
       <?php if($k%3 == 0): ?><div style='width:100%; clear:both;'></div>
       <?php else: ?>
        <div style="width:2%; height:50px; float:left;"></div><?php endif; endforeach; endif; else: echo "" ;endif; ?>
    <div style="clear:both;"></div>
    </div>
    <div class="item-list"><?php echo ($page); ?></div>
</div>
