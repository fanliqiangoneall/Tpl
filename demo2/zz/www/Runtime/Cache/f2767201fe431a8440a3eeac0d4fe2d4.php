...
<div class="r_mbx"><img src="__TMPL__demo2/img/in_home.jpg" border="0" style="vertical-align:middle;" />&nbsp;<a href="<?php echo U('/');?>">Home</a>&nbsp;>&nbsp;Sitemap</div>
...
<div class="page_Main">
    <div class="page_RightTitle">Sitemap</div>
    <div class="page_RightMain" id="sitemap" style="margin-top:30px;">
        <ul>
          <?php if(is_array($web_navigation)): $key = 0; $__LIST__ = array_slice($web_navigation,0,6,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_map): $mod = ($key % 2 );++$key;?><li><a href="<?php echo ($vo_map['url']); ?>" <?php if(strstr($vo_map['url'],'/contactus.html') == '/contactus.html'): ?>onclick="turn_sta(2,5)"<?php endif; ?>><?php echo ($vo_map['title']); ?></a>
             <?php if(strstr($vo_map['url'],'/product.html') == '/product.html'): ?><ul style="margin-bottom:10px;">
                  <?php if(is_array($allclass)): $i = 0; $__LIST__ = $allclass;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$allclass_map): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('/product_category','','');?>/<?php echo getprocatename($allclass_map['pcid']);?>"><?php echo ($allclass_map['class_name']); ?></a>
                        <?php if($allclass_map['childs'] != ''): ?><ul>
                            <?php if(is_array($allclass_map['childs'])): $i = 0; $__LIST__ = $allclass_map['childs'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$allclass_mapchild): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('/product_category','','');?>/<?php echo getprocatename($allclass_mapchild['pcid']);?>"><?php echo ($allclass_mapchild['class_name']); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
                          </ul><?php endif; ?>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
               </ul><?php endif; ?>
             <?php if($vo_map['child'] != ''): ?><ul style="margin-bottom:10px;">
                  <?php if(is_array($vo_map['child'])): $i = 0; $__LIST__ = $vo_map['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$navchild): $mod = ($i % 2 );++$i;?><li><a href="<?php echo ($navchild['url']); ?>"><h3><?php echo ($navchild['title']); ?></h3></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
               </ul><?php endif; ?>
           </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
     </div>
...
jQuery(document).ready(function(){
  jQuery(".sidebar ul li").hover(function(){
    jQuery(this).find(".sidebox").stop().animate({"width":"230px"},200).css({"opacity":"1","filter":"Alpha(opacity=100)","background":"#000000"})
