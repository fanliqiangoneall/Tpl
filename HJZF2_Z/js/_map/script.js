...
const animateLoop = (time) => {
  ...
  continentNames.forEach((name) => {
    // 大陆厚度
    let paths = getLandMassPaths(name, 600, 10);
...
const drawMarkers = (ctx, markers, drawFront) => {
  for (const marker of markers) {
    let ground = latLongSphere(marker.lat + $.scroll.lat, marker.long + $.scroll.long, 600),
        needleTop = latLongSphere(marker.lat + $.scroll.lat, marker.long + $.scroll.long, 610),
        pinTop = latLongSphere(marker.lat + $.scroll.lat, marker.long + $.scroll.long, 610);
...
const drawMarkerText = (ctx, text, pos) => {
    ctx.font = "80px PoppinsExtraLight";
    ctx.fillStyle = 'black';
