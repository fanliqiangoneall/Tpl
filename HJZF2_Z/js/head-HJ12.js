require(['jquery', 'swiper'], function ($, Swiper) {
    ...
    $(document).click(function (){
        $('.head_hj12_contact_nav').next().slideUp();
    }).on('click', '.head_hj12_contact_nav' , function (e) {
        e.preventDefault();
        $(this).toggleClass('active');
    ...
    $('.nav>ul>li').hover(function () {
        $('.head_hj12_fix_empty').addClass('active');
    }, function () {
        $('.head_hj12_fix_empty').removeClass('active');
    });
    function addActive() {
        var top = $(window).scrollTop();
        if (top > 150) {
            $('.head_hj12_fix.open').addClass('active');
        }else{
            $('.head_hj12_fix.open').removeClass('active');
        }
    };
    addActive();
