;(function (factory) {
	...
	$.fn.imgSwitch = function (opt) {
		//imgSwitch
		var self;
		var imgSwitch = function(ele, opt) {
			self = this;
			this.$element = ele,
				this.defaults = {
					'Auto':true,
			...
			this.arrImgs=this.$element.children("img");
			this.index = 0;
			this.Case=[[32,1,1],[4,4,2],[16,1,1],[1,1,0],[1,1,1],[1,1,2],[1,1,3],[1,1,4],[1,1,5],[1,1,6],[1,8,2],[4,2,2],[4,2,0],[8,3,0],[4,2,5],[4,1,5],[4,1,3],[1,4,4]];
			this.pos=0;
			this.nums=[];
		...
		imgSwitch.prototype = {
			createMask:function(){
				...
				this.$element.append(this._mask);
			},
			setMask:function(bigImg,col,row){

			/*	console.log(bigImg+'---4'+col+'----4'+row);*/
			...
			timeLine:function(play,end,len){
				...
				var s = 0;t=0;
				c = function(f,t){return +f+(t-f)*(s>1?s-Math.floor(s)==0?1:s-Math.floor(s):s)};
			...
			fxs:[
				function(el,x){this.clipDiv(el,x(this.maskHeight,0),x(0,this.maskWidth),x(0,this.maskHeight),x(this.maskWidth,0))},//閿熸枻鎷烽敓杈冭鎷烽敓鎹风》鎷烽敓鏂ゆ嫹閿熸枻鎷烽敓閾拌鎷烽敓鏂ゆ嫹閿熸枻鎷?
			...
			start:function(){
				...
				self.timer = self.timeLine(
					...
					function (){
						self.$element[0].style.background='url('+cur+')';
			...
			createPagebar:function(){
				...
				if(self.Navigate !="no"){
					...
					if((self.PicturePosition =="bottom" || self.PicturePosition =="right") && self.Navigate =="picture" && self.NavigatePlace == "outer"){
						self.parentNode.appendChild(pageBar);
					}else if(self.PicturePosition =="left" && self.Navigate =="picture" && self.NavigatePlace == "outer"){
						self.parentNode.insertBefore(pageBar,self);
					}
					else{
						this.$element.append(pageBar);
					}
				}
				...
				if(self.Navigate=="picture"){
					...
					for(var i = 0;i<self.len;i++){
						...
						if(self.PicturePosition=="bottom" && self.NavigatePlace=="outer"){
							...
							self.style.cssText ="position:relative;left:0;";
		...
		var imgSwitch = new imgSwitch(this, opt);
		return imgSwitch;
