require(['jquery','swiper'],function($,swiper){
    ...
    $("#HJZF2_advantage12__wrap li").each(function(){
    ...
    function mouseEvent(angle,o,d){ //方向  元素  鼠标进入/离开
        ...
        if(d=='in'){
            switch(angle){
                case '上':
                    $(o).find(".HJZF2_advantage12__after").css({left:0,top:-h+"px"}).stop(true).animate({left:0,top:0},300)
                   /* setTimeout(function(){
                        $(o).find("p a").css({left:'50%',top:-h+"px"}).stop(true).animate({left:'50%',top:'20px'},300)
                    },200)*/
                    break;
                case '右':
                    $(o).find(".HJZF2_advantage12__after").css({left:w+"px",top:0}).stop(true).animate({left:0,top:0},300)
                    /*setTimeout(function(){
            ...
            switch(angle){
                case '上':
                   /* $(o).find("p a").stop(true).animate({left:'50%',top:-h+"px"},300)*/
                    setTimeout(function(){
                        $(o).find(".HJZF2_advantage12__after").stop(true).animate({left:0,top:-h+"px"},300)
                    },200)
                    break;
                case '右':
                   /* $(o).find("p a").stop(true).animate({left:w+"px",top:'20px'},300)*/
                    setTimeout(function(){
                        $(o).find(".HJZF2_advantage12__after").stop(true).animate({left:w+"px",top:0},300)
                    },200)
                    break;
                case '下':
                   /* $(o).find("p a").stop(true).animate({left:'50%',top:h+"px"},300)*/
